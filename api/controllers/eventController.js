...
exports.addEventComment = async (req, res) => {
  ...
  try {
    ...
    await Event.findOneAndUpdate({ _id: req.params.id }, { $push: { comments: comment } }).catch((error) => {
      throw new Error('There is no event with this ID');
    });
  } catch (error) {
    return res.status(400).json({ error: error.message });
...
exports.deleteEventComment = async (req, res) => {
  try {
    await Event.findOneAndUpdate({ _id: req.params.id }, { $pull: { comments: { _id: req.params.commentId } } });
  ...
  return async (req, res, next) => {
    try {
      ...
      Event.create(event, (err, item) => {
        ...
        User.findOneAndUpdate({ _id: req.user }, { $push: { events: item } }).catch((error) => {
          throw new Error('There is no user with this ID');
        });

        Organization.findOneAndUpdate({ _id: req.body.organization }, { $push: { events: item } }).catch((error) => {
          throw new Error('There is no organization with this ID');
        });
